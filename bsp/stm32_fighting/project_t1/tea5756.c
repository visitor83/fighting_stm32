...
static rt_uint8_t Tx1_Buffer[] = {0XF0,0X2C,0XD0,0X12,0X40};
 ...
 * @param freq Unit MHz
 */
void tea5756_tune_pll(float freq, enum tea5756_mode opermode)
{
    ...
    pll =(rt_uint32_t)((4 *(freq * 1000000 + fIf * 1000))/fRef);

    if(opermode ==  TEA5756_SEARCH) {
        Tx1_Buffer[0] = (rt_uint8_t)((pll >> 8) & 0x3f | TEA5767_MUTE | TEA5767_SEARCH);
    } else {
        Tx1_Buffer[0] = (rt_uint8_t)((pll >> 8) & 0x3f);
    }
    Tx1_Buffer[1]= (rt_uint8_t) (pll & 0x0f);                       //I2C第二字节值
    I2C_Write(&Tx1_Buffer[0], Tea5767_WriteAddress1, 5); 
    rt_sem_release(&tea5756_device.sem_lock);
}

void tea5756_fix_freq(rt_uint32_t freq)
{
    tea5756_tune_pll(freq, TEA5756_PRESET);
 ...
 */
        ...
        if((Rx1_Buffer[0]&0x3f)!=(Tx1_Buffer[0]&0x3f)||(Rx1_Buffer[1]!=Tx1_Buffer[1])||(Rx1_Buffer[1]&0x80!=0x80)||Rx1_Buffer[2]<50||Rx1_Buffer[2]>=56||(Rx1_Buffer[3]>>4)<7||(Rx1_Buffer[3]>>4)>14){
            ...
            printf("\n 当前FM频率是:   %u MHz     有信号!!!    %u  %u\n",
                           a * 10 , Rx1_Buffer[2],Rx1_Buffer[3]>>4);
            fmCh[ch2]= fmreq;                    
            ch2++;
            continue;
        }
...
void tea5767_status_dump(rt_uint8_t *_buffer)
{
    unsigned int pll_div, freq;

    rt_uint8_t  buffer[5];
    
    I2C_ReadByte(buffer, 5, Tea5767_ReadAddress1);
...
rt_bool_t tea5767_autodetection()
{
    ...
    if (buffer[0] == buffer[1] && buffer[0] == buffer[2] &&
        ...
        rt_kprintf("All bytes are equal. It is not a TEA5767\n");
        return RT_FALSE;
    }
    /*  Status bytes:
    ...
    if (((buffer[3] & 0x0f) != 0x00) || (buffer[4] != 0x00)) {
        rt_kprintf("Chip ID is not zero. It is not a TEA5767\n");
        return RT_FALSE;
    }              
    return RT_TRUE;
}
...
//    tea5756_device.FM_FREQ = 988 * FREQ_1MHz;
    ...
    //tea5756_tune_pll(98.8,  TEA5756_IDLE);
}
