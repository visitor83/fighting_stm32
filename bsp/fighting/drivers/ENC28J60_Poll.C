...
extern void delay_ms(rt_uint32_t ms);
...
//元。
	...
	rt_kprintf("MAADR5 = 0x%x\r\n", enc28j60Read(MAADR5));
	rt_kprintf("MAADR4 = 0x%x\r\n", enc28j60Read(MAADR4));
	rt_kprintf("MAADR3 = 0x%x\r\n", enc28j60Read(MAADR3));
	rt_kprintf("MAADR2 = 0x%x\r\n", enc28j60Read(MAADR2));
	rt_kprintf("MAADR1 = 0x%x\r\n", enc28j60Read(MAADR1));
	rt_kprintf("MAADR0 = 0x%x\r\n", enc28j60Read(MAADR0));
	}
	//配置PHY为全双工  LEDB为拉电流
...
unsigned int enc28j60PacketReceive(unsigned int maxlen, unsigned char* packet)
    ...
    if ((rxstat & 0x80)==0)
			...
			rt_kprintf("\nrxstat:0x%08x, len:%d", rxstat, len);
